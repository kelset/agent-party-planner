...
- **File/Context Injection:** Does *not* have a `--system-prompt` flag. Context must be injected by reading the file into the initial prompt string (e.g., `-i "Act as Game Creator. Read this: $(cat .orchestration/throne_room/game-creator.md)"`).
...
- *(Needs detailed flag mapping once API is confirmed)*
