...
_(For full details, see the [Orchestration Levels](./orchestration/ORCHESTRATION_LEVELS.md), [The Throne Room](./orchestration/THRONE_ROOM.md), [Game Master Guide](./orchestration/GAME_MASTER_GUIDE.md), and [Party Roles](./orchestration/PARTY_ROLES.md) documents.)_

1. **The Throne Room (Meta-Level):**
   - The highest level of orchestration.
   - Users select their "Meta-Agents" or "Throne Room" members (e.g., the primary agent they are interacting with).
...
1. **Initialization:** A user facing a complex task creates a new, empty directory for the orchestration (e.g., `./agent-orchestration`). They run a command here (e.g., `app-cli start-throne-room`) to spin up a Throne Room session. This connects the user with the Game Creator in an isolated environment.
2. **Brainstorming:** In the Throne Room, the user and the Game Creator define the task, party composition, and overarching goals. They generate a "quest folder" inside this orchestration directory containing the base rules and parameters.
