...
export function generateThroneRoomPrompt(config: OrchestrationConfig): string {
  const { throneRoom } = config;

  return `# The Throne Room

The **Throne Room** is the highest level of orchestration in the Agents Party for the quest: **${config.questName}**. It serves as the meta-level interface between the human user and the underlying execution layers (the GM Session and the Party).
...
The Throne Room is designed to be the first point of entry for a complex task. It must be run in a **separate orchestration directory**, completely outside of the target codebase repository where the actual coding will occur. This prevents the codebase from being cluttered with agent journals and meta-prompts.
...
There are moments when the GM will naturally pause (e.g., hitting a 36-hour Time-to-Live limit or running low on agent tokens). These forced pauses are the perfect moments for the user to return to the Throne Room. The user can consult with the Game Creator or the Master of Spies to assess the situation and push new rules to \`active-rules.md\` before telling the GM to resume. Additionally, if the Throne Room decides a quest is beyond saving, the user can execute an abort command to signal the GM to hard-reset the workspace and terminate.
`;
}
